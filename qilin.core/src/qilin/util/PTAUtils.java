...
//        }
    ...
    public static boolean addWithoutTypeFiltering(PointsToSetInternal pts, Node node) {
        return pts.add(node.getNumber());
    }
    ...
    public static boolean isConcerned(Type type) {
        return type instanceof RefLikeType && !(type instanceof NullType);
    }


    public static ValNode getCsInstanceField(PTA pta, AllocNode base, SparkField fieldOrArrIdx){
        final FieldValNode fvn = pta.getPag().makeFieldValNode(fieldOrArrIdx);
        return (ValNode) pta.parameterize(fvn, PTAUtils.plusplusOp(base));
    }
}