 ...
 */
public class CSCPAG extends PAG{
    ...
    private CutShortcutSolver cutShortcutSolver;

    public void setCutShortcutSolver(CutShortcutSolver solver) {
        this.cutShortcutSolver = solver;
    }

    public CSCPAG(PTA pta) {
        super(pta);
        this.pta = pta;
...
//            cutShortcutSolver.addPTSOnNewPagEdge(from, to);
//            System.out.println("#addPFGEdge:\t" + from + "\t=>" + to);

            if(cutShortcutSolver != null) {
                cutShortcutSolver.onNewPFGEdge(from, to);
            }
            edgeQueue.add(from);
            edgeQueue.add(to);
        }
    ...
    public Set<ValNode> succNodeOfHost(HostPointer n) {
        return simpleLookup(n);
    }
    ...
    public Set<Node> predNodesOfCSVar(VarNode n){
        Set<Node> preds = new HashSet<>(simpleInvLookup(n));
        preds.addAll(loadInvLookup(n));
        preds.addAll(allocInvLookup(n));
        return preds;
    }

    public Set<Node> succNodesOfCSVar(VarNode n){
        Set<Node> succs = new HashSet<>(simpleLookup(n));
        succs.addAll(storeLookup(n));
        return succs;
    }
    ...
    public MethodPAG getMethodPAG(SootMethod m) {
        ...
        synchronized (body) {
            ...
            if (m.isConcrete()) {
//                System.out.println("Warning: disable reflection analysis");
            ...
            ret = methodToPag.computeIfAbsent(m, k -> new MethodPAG(this, m, body));
        }
        return ret;
    }
