 ...
 */
public class CSCMethodNodeFactory extends MethodNodeFactory {
    protected CSCPAG pag;
    protected MethodPAG mpag;
    protected SootMethod method;

    private Set<LocalVarNode> realReturnVars;
    private LocalVarNode realThisVars;
    private LocalVarNode[] realParmVars;

    private final MultiMap<LocalVarNode, AssignStmt> varToInvokeWithReturn = Maps.newMultiMap();

    public CSCMethodNodeFactory(CSCPAG pag, MethodPAG mpag) {
        super(pag, mpag);
        this.pag = pag;
        this.mpag = mpag;
    ...
    public Set<AssignStmt> getInvokesWithReturn(LocalVarNode var){
        return varToInvokeWithReturn.get(var);
    }
    /**
    ...
    protected void handleInvokeStmt(Stmt s) {
        ...
        if (ie instanceof InstanceInvokeExpr) {
            LocalVarNode base = (LocalVarNode) getNode(((InstanceInvokeExpr) ie).getBase());
            if(s instanceof AssignStmt stmt){
                varToInvokeWithReturn.put(base, stmt);
            }
    ...
    private void handleIntraStmt(Stmt s) {
        s.apply(new AbstractStmtSwitch<>() {
            ...
            public void caseReturnStmt(ReturnStmt rs) {
                ...
                if(realReturnVars == null){
                    realReturnVars = new HashSet<>();
                }
                realReturnVars.add((LocalVarNode)retNode);
            ...
            public void caseIdentityStmt(IdentityStmt is) {
                ...
                if(src.equals(caseThis())){
                    if(realThisVars == null){
                        realThisVars = (LocalVarNode) dest;
                    }
                    else{
                        throw new RuntimeException("Multiple this variables in a method.");
                    }
                }
                ...
                for (int i = 0; i < method.getParameterCount(); i++) {
                    ...
                    if(src.equals(caseParm(i))){
                        if(realParmVars == null){
                            realParmVars = new LocalVarNode[method.getParameterCount()];
                        }
                        realParmVars[i] = (LocalVarNode) dest;
                    }
                }
    ...
    public boolean isRealReturnVar(LocalVarNode n){
        if(method.isAbstract()) return false;
        return realReturnVars != null && realReturnVars.contains(n);
    }

    public boolean isRealThisVar(Node n){
        return realThisVars != null && realThisVars.equals(n);
    }

    public boolean isRealParmVar(Node n){
        return realParmVars != null && List.of(realParmVars).contains(n);
    }

    public boolean isRealParmVar(Node n, int index){
        return realParmVars != null && n.equals(realParmVars[index]);
    }
    public LocalVarNode getRealParmVars(int index){
        if(realParmVars == null) return null;
        if(index >= method.getParameterCount()){
            throw new RuntimeException("Invalid index for realParmVars.");
        }
        return realParmVars[index];
    }

    public LocalVarNode getRealThisVar(){
        return realThisVars;
    }


    public Set<LocalVarNode> getRealReturnVars() {
        return realReturnVars == null ? Set.of(): realReturnVars;
    }
