...
def runPTA(analysis, bm, OPTIONSTYLE):
    ...
    if (analysisName in OoM and bm in OoM[analysisName]) or (
        analysisName in TIMEOUT and bm in TIMEOUT[analysisName]
    ):
...
OPTIONMESSAGE = (
    ...
    + option(
        "-cda=<[CONCH|DEBLOATERX|COLLECTION]>",
    ...
    + option(
        "-cga=<[CHA|VTA|RTA|GEOM|SPARK|QILIN]>",
    ...
    + option(
        "-all",
        "run all analyses for specified benchmark(s) if ONLY benchmark(s) is specified;\n\
...
if __name__ == "__main__":
    ...
    if len(benchmarks) == 0:
        sys.exit(ALLBENCHMARKS)
        sys.exit("run benchmark(s) not specified.")  # + OPTIONMESSAGE)
    if len(analyses) == 0:
