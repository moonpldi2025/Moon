...
public class DebloaterX {
    ...
    private Set<AllocNode> preCtxDepHeaps = Set.of();
    public DebloaterX(PTA pta, Set<AllocNode> ctxDepHeaps){
        this(pta);
        this.preCtxDepHeaps = ctxDepHeaps;
    }
    ...
    public Set<AllocNode> getPureInnerContainer(){
        return Sets.difference(innerContainer, Sets.union(containerFactory, containerWrapper));
    }
    /*
    ...
    public void run() {
        ...
        Collection<AllocNode> allocNodes = pag.getAllocNodes();
        if(!preCtxDepHeaps.isEmpty()){
            allocNodes = allocNodes.stream().filter(preCtxDepHeaps::contains).toList();
        }
        for (AllocNode heap : allocNodes) {
            SootMethod method = heap.getMethod();
